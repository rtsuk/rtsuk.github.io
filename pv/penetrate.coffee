...
calculateOne = (attackDice, crits, hits, defenseDice, shields, firepower, armor, cancelCrits) ->
	...
	if critCount > 0
		...
		if result > armor
			penetrate = true
	hit: (hitCount > 0 || critCount > 0), penetrate: penetrate, hitCount: totalHits, critCount: totalCrits, shieldCount: shieldCount
...
calculate = () ->
	...
	for armor in [1...20]
		...
		for i in [1..iterations] by 1
			{hitCount, critCount, shieldCount, hit, penetrate} = calculateOne attackDice, critSides, hitSides, defenseDice, shields, firepower, armor, cancelCritsFirst
			totalHits += hitCount
			totalCrits += critCount
